...
def test_playerid_lookup():
    ...
    result1 = client.search("Ramirez", ignore_accents=True)
    assert isinstance(result1, pd.DataFrame)
    assert len(result1) > 0

    # Test case 2: Lookup player by last name and first name
    result2 = client.search("Ramirez", "Jose", ignore_accents=True)
    assert isinstance(result2, pd.DataFrame)
    assert len(result2) > 0

    # Test case 3: Lookup player with non-existent last name
    result3 = client.search("NonExistentLastName", ignore_accents=True)
    assert isinstance(result3, pd.DataFrame)
    assert len(result3) == 0

    # Test case 4: Lookup player with non-existent first name
    result4 = client.search("Doe", "NonExistentFirstName", ignore_accents=True)
